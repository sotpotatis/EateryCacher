...
class MenuParser:
    ...
    def parse(self, menu_content):
        ...
        menu_metadata = {"title": menu_title, "week_number": menu_week} #The menu data result
        result = {}
        ...
        for row in raw_menu_lines:
            ...
            for day, day_id in day_names_to_json_keys.items():
                if day in row: #If a day was found
                    ...
                    break
            if found_day == None and current_day != None: #If no day was found in this line, add the menu content to another line
                ...
                for special_feature, special_feature_key in special_features.items():
                    if special_feature in row:
                        ...
                        result[current_day]["special_features"][special_feature_key] = True  #Mark that an attribute was found
                    else:
                        logger.debug(f"{special_feature} was not found for {current_day} (row content: {row})")
                logger.debug(f"Special features for {current_day}: {result[current_day]['special_features']}") #Log found special features for the day
                if len(row) > 0:
                    if any(re.fullmatch(regex, row) for regex in known_footer_phrases) is False: #If no known footer phrases has been found
